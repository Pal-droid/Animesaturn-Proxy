    ...
    if is_m3u8:
        ...
        def rewrite_match(uri):
            if not uri.startswith("http"):
                ...
                return f"/api/main/proxy?url={quote(abs_uri, safe='')}"
            return uri
...
const source = "/api/main/proxy?url={video_url}";
if(source.endsWith(".m3u8")){{
    if(Hls.isSupported()){{const hls=new Hls();hls.loadSource(source);hls.attachMedia(video);hls.on(Hls.Events.MANIFEST_PARSED,()=>{{video.play();}});}}
    else if(video.canPlayType('application/vnd.apple.mpegurl')){{video.src=source;video.addEventListener('loadedmetadata',()=>{{video.play();}});}}
    else{{document.body.innerHTML="<h3 style='color:white'>Browser cannot play HLS streams.</h3>";}}
}}else{{video.src=source;video.play();}}
</script>
</body></html>"""
